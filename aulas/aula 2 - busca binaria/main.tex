...
\begin{frame}{Fundamento: Busca em Lista Ordenada}
	...
	\begin{center}
		...
		\sum_{j=i+1}^{\,n - i}
		\left\lfloor \frac{n - i}{j} \right\rfloor
		\]
...
\begin{frame}{Fundamento: Busca em Lista Ordenada}
	...
	\begin{itemize}
			\pause
		\item Complexidade de tempo: \textbf{$\mathcal{O}{(N + Q)}$} \pause \textcolor{darkgreen}{(OK)}.
			\pause
...
\begin{frame}{Generalizando: \textbf{Monotonicidade}}
	\textcolor{structure}{Exemplo: $\mathcol{f}{(x)} = \lfloor x \rfloor$}
	\begin{center}
		\begin{tikzpicture}
...
\begin{frame}{Generalizando: \textbf{Monotonicidade}}
	\textcolor{structure}{Exemplo: $\mathcol{f}{(x)} = \frac{x(x - 1)}{2}$}
	\begin{center}
		\begin{tikzpicture}
...
\begin{frame}{Generalizando: \textbf{Monotonicidade}}
	\textcolor{structure}{Exemplo: $\mathcol{f}{(x)} = \max\left(1, \frac{(x - 5)^3}{3} + x\right) + 2(\lfloor \frac{x}{2} \rfloor)$}
	\begin{center}
		\begin{tikzpicture}
...
\begin{frame}{Busca binária na maratona}
	\begin{itemize}
		\item Resumindo: 
			...
			\begin{center}
				\begin{tcolorbox}[
					...
					\begin{itemize}
						\item Primeiro ponto tal que $\mathcol{f}{(x)} \leq Y$ ou,
						\item Último ponto tal que $\mathcol{f}{(x)} > Y$ ou,
		...
		\item A dificuldade dos problemas usualmente é:
			\begin{itemize}
					\pause
				\item Notar que uma	$\mathcol{f}{(x)}$, útil para resolver o problema, é monotônica.
					\pause
				\item Achar um jeito esperto de computar $\mathcol{f}{(x)}$.
			\end{itemize}
