...
int main() {
	...
	function<ll(int, int)> calc = [&](int l, int r) -> ll {
		ll mnpre = l ? min_pre[l - 1] : LLONG_MAX;
		ll mnsuf = r + 1 < n ? min_suf[r + 1] : LLONG_MAX;
		//cout << "jan: " << mnpre << endl;
		//cout << "pre: " << mnsuf << endl;
		//cout << "suf: " << F.begin()->first - off << endl;

		ll agr = min({mnpre, mnsuf, F.begin()->first - off++});
		//cout << "agr: " << agr << endl;
	...
	for (int i = k; i < n; i++) {
		...
		//cout << calc(i-k, i) << endl;
		B.push_back(calc(i-k, i));
